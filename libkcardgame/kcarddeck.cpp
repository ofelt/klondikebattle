...
QString KCardDeck::suitToString( int suit ) const
{
    switch ( suit )
    {
...
QString KCardDeck::rankToString( int rank ) const
{
    switch ( rank )
    {
    case KCardDeck::Jack:
...
QString KCardDeck::elementName( quint32 id, bool faceUp ) const
{
    ...
    const int rank = rankFromId( id );
    const int suit = suitFromId( id );

    return rankToString(rank) + "_" + suitToString(suit);
}
