...
void Krapette::initialize()
{
    ...
    reserveJ1->setPileRole( PatPile::Reserve );
    reserveJ1->setLayoutPos( dist_x * 2, 0 );
    reserveJ1->setKeyboardSelectHint( KCardPile::ForceFocusTop );
    reserveJ1->setKeyboardDropHint( KCardPile::NeverFocus );
    
    m_player1 = new Player(reserveJ1, talonJ1, wasteJ1, 
                            true, Settings::player1Name());
    ...
    reserveJ2->setPileRole( PatPile::Reserve );
    reserveJ2->setLayoutPos( dist_x * 7, 0 );
    reserveJ2->setKeyboardSelectHint( KCardPile::ForceFocusTop );
    reserveJ2->setKeyboardDropHint( KCardPile::ForceFocusTop );
    
    m_player2 = new Player(reserveJ2, talonJ2, wasteJ2, 
                            Settings::player2Human(), Settings::player2Name());
    
    for (int i = 0; i < 8; ++i )
    {
        m_target[i] = new PatPile( this, i + 6, QString("target%1").arg(i + 6) );
        m_target[i]->setPileRole(PatPile::Foundation);
        m_target[i]->setLayoutPos(dist_x*(3+(i/4)), dist_y+(i%4)*dist_y);
        m_target[i]->setSpread(0, 0);
        m_target[i]->setKeyboardSelectHint( KCardPile::NeverFocus );
        m_target[i]->setKeyboardDropHint( KCardPile::ForceFocusTop );
    }
    for (int i = 0; i < 4; ++i )
    {
        m_play[i] = new PatPile( this, i + 10, QString( "play%1" ).arg( i + 10));
        ...
        m_play[i]->setKeyboardSelectHint( KCardPile::AutoFocusDeepestRemovable );
        m_play[i]->setKeyboardDropHint( KCardPile::AutoFocusTop );
    }
    for (int i = 4; i < 8; ++i )
    {
        m_play[i] = new PatPile( this, i + 14, QString( "play%1" ).arg( i + 14));
        m_play[i]->setPileRole(PatPile::Tableau);
...
bool Krapette::checkAdd(const PatPile *pile, const QList<KCard*> &oldCards, const QList<KCard*> &newCards) const
{
    ...
    case PatPile::Waste:{
        ...
        if(pile == getActiveWaste()) {
            // Forbid to put a card on our waste if there is empty tabled piles
            if(emptyPlayPiles > 0) {
                return false;
            }
        ...
        return newCards.count() == 1 
                && pile != getActiveWaste() 
                && (isSameSuitClose(oldCards.last(), newCards.first()));
    }
    case PatPile::Reserve:{
        return newCards.count() == 1 
                && !pile->isEmpty()
...
bool Krapette::checkRemove(const PatPile *pile, const QList<KCard*> &cards) const
{
    ...
    {
    case PatPile::Tableau:
        return getActiveTalon()->isEmpty() || !getActiveTalon()->topCard()->isFaceUp();
    case PatPile::Foundation:
        return false;
    case PatPile::Waste:
        return false;
    case PatPile::Reserve:
        return pile == getActiveReserve() 
            && (getActiveTalon()->isEmpty() || !getActiveTalon()->topCard()->isFaceUp());
    case PatPile::Stock:
        return true;
    default:
...
void Krapette::cardsMoved( const QList<KCard*> &cards, KCardPile *oldPile, KCardPile *newPile )
{
    ...
    if (oldPatPile->pileRole() == PatPile::Reserve && !oldPatPile->isEmpty()) {
        oldPile->topCard()->setFaceUp(true);
    }

    // Put face down foundation pile completed
    // Not working here so we do it in animationDone
    //if(newPatPile->pileRole() == PatPile::Foundation
    //     && cards.first()->rank() == KCardDeck::King) {
    //     newPile->topCard()->setFaceUp(false);
    // }
...
void Krapette::animationDone()
{
    ...
    // Put face down foundation pile completed
    // Could be done in cardsMoved but not working there
    PatPile* foundation;
    for (int i = 0; i < 8; i++) {
        foundation = m_target[i];
        if(!foundation->isEmpty() && foundation->topCard()->rank() == KCardDeck::King) {
            foundation->topCard()->setFaceUp(false);
        }
    }
...
bool Krapette::newCards()
{
    ...
    if (getActiveTalon()->isEmpty()) {
        ...
        if (getActiveTalon()->topCard()->isFaceUp()) {
            return false;
        }  
...
bool Krapette::checkCompulsoryMoves() const
{
    if (m_compulsoryMovesEnabled) {
        for (int i = 0; i < 8; i++) {
            if (!getActiveTalon()->isEmpty() 
                && getActiveTalon()->topCard()->isFaceUp() 
                && allowedToAdd(m_target[i], getActiveTalon()->topCard())) {
                    return true;
            }
            if (!getActiveReserve()->isEmpty() 
                && allowedToAdd(m_target[i], getActiveReserve()->topCard())) {
                return true;
            }
            
            // Check if we can add a card to the foundation from tabled cards
            for (int j = 0; j < 8; j++) {
                if (!m_play[j]->isEmpty() && allowedToAdd(m_target[i], m_play[j]->topCard())) {
                    return true;
                }
            }
        }
...
Player* Krapette::getOpponent() const
{
...
void Krapette::tryMoveAI()
{
    ...
    if (!getActiveTalon()->isEmpty() && getActiveTalon()->topCard()->isFaceUp()) {
        ...
        if (putCardToPile(cardToPlay, getOpponent()->reserve())) {
            return;
        }
        if (putCardToPile(cardToPlay, getOpponent()->waste())) {
            return;
        }
        for (int i = 0; i < 8; i++) {
            if (putCardToPile(cardToPlay, m_play[i])) {
                return;
            }
        }
        // no other choices discard
        moveCardToPile( cardToPlay, getActiveWaste(), m_aiDurationMove );
        return;
    }
    ...
    if (!getActiveReserve()->isEmpty()) {
        for (int i = 0; i < 8; i++) {
            if (putCardToPile(getActiveReserve()->topCard(), m_play[i])) {
                return;
            }
        }
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        /*const QVector v = m_cardsPlayed;
        v.append(m_play[i]->topCard());
        if(hasLoop(v)) {
            kWarning() << "detectCycle";
            //continue;
        }*/
        ...
        for (int j = 0; j < 8; j++) {
            ...
            if (m_play[i] != m_play[j] && !isMoveUseless(m_play[i], m_play[j])
                && putCardToPile(m_play[i]->topCard(), m_play[j])) {
                return;
            }
        }
    }
...
{   
    if (allowedToAdd(getOpponent()->reserve(), card)) {
        return true;
    }
    if (allowedToAdd(getOpponent()->waste(), card)) {
        return true;
    }
    
    for (int i = 0; i < 8; i++) {
        if (!m_play[i]->isEmpty() && card->pile() != m_play[i]
            && allowedToAdd(m_play[i], card)) {
            return true;
        }
    }
...
bool Krapette::doCompulsoryMoves() 
{
     //if (m_compulsoryMovesEnabled) {
        for (int i = 0; i < 8; i++) {
            if (!getActiveTalon()->isEmpty() && getActiveTalon()->topCard()->isFaceUp()
                && putCardToPile(getActiveTalon()->topCard(), m_target[i])) {
                return true;
            }
            if (!getActiveReserve()->isEmpty() 
                && putCardToPile(getActiveReserve()->topCard(), m_target[i])) {
                return true;
            }
            
            // Check if we can add a card to the foundation from tabled cards
            for (int j = 0; j < 8; j++) {
                if (!m_play[j]->isEmpty() && putCardToPile(m_play[j]->topCard(), m_target[i])) {
                    return true;
                }
            }
        }
...
bool Krapette::putCardToPile(KCard * card, PatPile * destPile)
{
    if (allowedToAdd(destPile, card)) {
        moveCardToPile( card, destPile, m_aiDurationMove );
        return true;
    }
    return false;
}

bool Krapette::putTabledCardToPlayerPile(PatPile * destPile)
{
    if (!destPile->isEmpty()) {
        for(int i = 0; i < 8; i++) {
            if (!m_play[i]->isEmpty() && putCardToPile(m_play[i]->topCard(), destPile)) {
                return true;
            }
        }
    }
